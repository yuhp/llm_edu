...
const copy = {
  en: {
    title: 'LLM App Architecture Evolution',
    subtitle: 'From passive inference to multi-agent delivery systems',
    badge: 'Course 01',
    stagePrefix: 'Chapter',
  ...
  return (
    <section className="space-y-5">
      <div className="rounded-3xl border border-slate-800 bg-slate-900/55 p-6 md:p-8">
        <div className="mb-3 inline-flex rounded-full border border-blue-500/30 bg-blue-500/10 px-3 py-1 text-xs font-semibold tracking-[0.2em] text-blue-300 uppercase">
          {t.badge}
        </div>
        <h2 className="text-3xl font-bold text-white md:text-4xl">{t.title}</h2>
        <p className="mt-3 max-w-4xl text-sm leading-7 text-slate-400 md:text-base">{t.subtitle}</p>
      </div>
            ...
            <button
              ...
              >
                <div className="flex items-center gap-3">
                  <span className={`rounded-md px-2 py-1 text-[10px] font-bold tracking-widest uppercase ${index === currentStageIndex ? 'bg-blue-500/20 text-blue-300' : 'bg-slate-800 text-slate-400'}`}>
                    {t.stagePrefix} {index + 1}
                  </span>
                <span className={`font-semibold ${index === currentStageIndex ? 'text-blue-100' : 'text-slate-300'}`}>
                  {stage.title.split(': ')[1] || stage.title}
                </span>
                </div>
                <div className="mt-3 text-sm text-slate-400">{stage.subtitle}</div>
              </button>
            ))}
        </nav>
      </div>
      ...
      <div className="grid gap-5 2xl:grid-cols-[minmax(0,1fr)_380px]">
        <div className="flex min-h-[760px] flex-col rounded-3xl border border-slate-800 bg-slate-900/55 p-5 md:p-6">
          ...
          <div className="mt-4 rounded-2xl border border-slate-800 bg-slate-950/50 p-4">
            <div className="mb-3 text-[11px] font-bold uppercase tracking-[0.25em] text-slate-500">{t.trend}</div>
            ...
            <div className="mt-2 grid grid-cols-4 gap-2 text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-500">
              {t.trendLabels.map((label, index) => (
                <span key={label} className={index <= currentStageIndex ? 'text-blue-300' : undefined}>
                  {label}
                </span>
              ))}
            </div>
        ...
        <aside className="flex flex-col rounded-3xl border border-slate-800 bg-slate-900/55 p-5 md:p-6">
          <h3 className="text-lg font-semibold text-white">{currentStage.subtitle}</h3>
          <p className="mt-2 text-sm leading-relaxed text-slate-400">{currentStage.description}</p>
          <div className="mt-5 text-xs font-bold uppercase tracking-[0.2em] text-slate-500">{t.capabilities}</div>
          ...
          <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/50 p-4 text-sm text-slate-400">
            ...
            <div className="mt-2">{t.stageLabel}: 0{currentStageIndex + 1}</div>
          </div>
          <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-950/50 p-4 text-sm text-slate-400">
            <div className="mb-2 font-semibold text-slate-200">{stageNote(lang, currentStageIndex)}</div>
...
function stageNote(lang: Lang, index: number) {
  if (lang === 'zh') {
    return ['观察重点：模型是否只能回答', '观察重点：谁在调用工具', '观察重点：是否形成循环', '观察重点：是否出现协作分工'][index];
  }
